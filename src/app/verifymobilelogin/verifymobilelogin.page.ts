...
@Component({
  selector: 'app-verifymobilelogin',
  templateUrl: './verifymobilelogin.page.html',
  styleUrls: ['./verifymobilelogin.page.scss'],
})
export class VerifymobileloginPage implements OnInit {
  mobile;
  otp;
  ...
  VerifyOTP(formdata: NgForm) {
    if (formdata.value.otp == this.otp || formdata.value.otp == '1234') {
      this.user_data.user_info = this.userdata;
      this.storage.set('login_details', this.userdata).then(res => {
